...
class SeaweedUpscalingModel:
    ...
    def __init__(self, path, initial_seaweed=1000,
        ...
        self.parameters["initial_area_built"]=initial_area_built # km2 that are already prepared for seaweed production
        self.parameters["initial_area_used"]=initial_area_used # km2 that are already used for seaweed production
        ...
        self.parameters["max_area"]=max_area # maximal area that is to be used for seaweed production km2
        self.parameters["initial_lag"]=initial_lag # days until production starts (meant to model how long it takes people 
        # to get their act together after a nuclear war)
        self.parameters["additional_saturation_time"]=additional_saturation_time # To accomodate for lost plants we assume 10 percent 
    ...
    def calculate_global_food_demand_parameters(self):
        ...
        self.parameters["daily_calories_needed"] = self.parameters["global_food_demand_in_catastrophe"] * (self.parameters["calories_from_seaweed"]/100)
        self.parameters["seaweed_needed"] = (self.parameters["daily_calories_needed"] / self.parameters["calories_per_wet_weight"]) # [T/day]
    ...
    def seaweed_growth(self, harvest_loss, initial_seaweed, initial_area_built, 
    ...
    def determine_productivity(self, growth_rate, harvest_loss, min_density, max_density, 
                            percent_usable_for_growth, days_to_run):
        """
        # Let the model run for one km² to determine the productivity per area and day and the harvest intervall
        """
        df = self.seaweed_growth(harvest_loss=harvest_loss, initial_seaweed=1, initial_area_built=1, 
    ...
    def run_model_for_set_of_growth_rates(self, growth_rates=[5,10,15, 25], days_to_run=365):
        """
        ...
        for growth_rate in growth_rates:
            ...
            productivity_day_km2 = self.determine_productivity(growth_rate_frac, self.parameters["harvest_loss"],
                                                            self.parameters["min_density"], self.parameters["max_density"], self.parameters["percent_usable_for_growth"], days_to_run)
            ...
            df = self.seaweed_growth(harvest_loss=self.parameters["harvest_loss"], 
